...
4. 使用现有 Excel 生成器按当前订单基线生成缺失表，保证格式、工作表、字段、颜色与正常输出一致。
5. 累计台账缺失时视为一次新的当前订单基线：累计已加工从 0 开始；随后主流程继续处理
...
from modules.process_parts import build_current_state, build_source_index
...
def _verify_generated(cumulative_path: Path, pending_path: Path) -> None:
...
def _build_baseline_files(source_records: list[dict], temp_dir: Path) -> tuple[Path, Path]:
    baseline_rows = build_current_state(source_records, existing_state={})
    note = (
        "自动自愈重建：正式台账缺失，依据“正在加工”当前全部订单原始汇总表建立基线；"
    ...
    _verify_generated(cumulative_path, pending_path)
...
def prepare_official_files(drive: DriveManager) -> dict[str, dict | None]:
    ...
    with tempfile.TemporaryDirectory() as temp_dir_str:
        ...
        cumulative_path, pending_path = _build_baseline_files(source_records, temp_dir)

        if _test_mode():
            logger.info(
                f"只读测试：已验证可从 {len(source_records)} 条当前订单零件重建 {missing}；"
                "未上传 Drive，也未执行后续正式写回。"
            )
            return official

        paths = {
            CUMULATIVE_NAME: cumulative_path,
