...
class DriveManager:
    ...
    def list_children(self, folder_id: str) -> list[dict]:
        ...
        while True:
            result = service.files().list(q=query, fields=fields, pageToken=page_token, pageSize=1000,
    ...
    def list_order_source_files(self) -> list[dict]:
        ...
        for item in self.list_children(self.working_folder_id):
            ...
            for child in self.list_children(folder_id):
                ...
                if child.get("mimeType") == FOLDER_MIME or not lower.endswith((".xlsx", ".xlsm", ".xls")):
                    continue
            ...
            if len(candidates) > 1:
                raise RuntimeError(f"订单目录存在多个原始汇总表候选，禁止自动猜测: {container_name} -> {[x.get('name') for x in candidates]}")
            if not candidates:
                continue
            child = candidates[0]
            result.append({**child, "order_container_name": container_name, "order_folder_id": folder_id,
    ...
    def list_pending_split_files(self) -> list[dict]:
        ...
        for item in self.list_children(self.split_folder_id):
            ...
            if item.get("mimeType") == FOLDER_MIME or "_完成" not in name:
                continue
    ...
    def upload_file(self, file_path: str, target_folder_id: str) -> dict:
        ...
        result = self._service().files().create(body={"name": os.path.basename(file_path), "parents": [target_folder_id]},
            media_body=media, fields="id,name,parents,modifiedTime,size,md5Checksum", supportsAllDrives=True).execute()
        self.stats["uploads"] += 1
        return result

    def update_file_content(self, file_id: str, file_path: str) -> dict:
        media = MediaFileUpload(file_path, resumable=False)
        result = self._service().files().update(fileId=file_id, media_body=media,
    ...
    def verify_remote_hash(self, file_id: str, local_path: str | Path) -> dict:
        local = self.local_hashes(local_path)
        remote = self.get_file(file_id, fields="id,name,parents,modifiedTime,size,md5Checksum")
        if not remote.get("md5Checksum"):
            raise RuntimeError(f"Drive未返回MD5，无法确认上传完整性: {remote.get('name') or file_id}")
        if str(remote["md5Checksum"]).lower() != local["md5"].lower():
            raise RuntimeError(f"Drive MD5校验失败: {remote.get('name') or file_id}")
        if remote.get("size") not in (None, "") and int(remote["size"]) != local["size"]:
            raise RuntimeError(f"Drive size校验失败: {remote.get('name') or file_id}")
        return {**remote, "local_sha256": local["sha256"], "local_md5": local["md5"]}

    def move_file(self, file_id: str, target_folder_id: str) -> dict:
        info = self._service().files().get(fileId=file_id, fields="parents", supportsAllDrives=True).execute()
        previous = ",".join(info.get("parents", []))
